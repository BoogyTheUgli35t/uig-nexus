...
//
// DEPRECATED / UNUSED: the client (src/routes/portal.signup.choose-division.tsx) no longer
// calls this function. It now calls the `registerUserDivisions` TanStack Start server
// function (src/lib/portal.functions.ts) plus the per-division `seed*Data` server functions
// directly, which do the same job as the seed*() helpers below. This file is kept only for
// history — if it is still deployed, remove it with `supabase functions delete
// signup_choose_division` to avoid maintaining two divergent copies of the signup-seeding
// logic (this one, for instance, is missing a `cooperatives` migration entirely and would
// error if invoked for the agritech division).
...
async function seedRealEstate(supabase: SupabaseClient, userId: string) {
  // Properties
  const properties = [
    {
      ...
      status: "available",
      description:
        "Automated 5-bedroom smart villa with solar backup, smart locks and a private cinema.",
      owner_id: userId,
    },
    {
  ...
  if (propertyData && propertyData.length > 0) {
    const propertyIds = propertyData.map((p: { id: string }) => p.id);
    ...
    const { error: tenantError } = await supabase.from("tenants").insert(tenantsWithProperty);
    ...
    const { error: leadError } = await supabase.from("leads").insert(leadsWithProperty);
  ...
  const { error: investorError } = await supabase.from("investors").insert(investorsWithOwner);
...
async function seedTechnology(supabase: SupabaseClient, userId: string) {
  // Projects
  const projects = [
    {
  ...
  if (projectData && projectData.length > 0) {
    ...
    const { error: taskError } = await supabase.from("tech_tasks").insert(tasks);
  ...
  const { error: integrationError } = await supabase.from("integrations").insert(integrations);
...
async function seedAgriTech(supabase: SupabaseClient, userId: string) {
  // Farmers
  const farmers = [
    {
  ...
  if (farmerData && farmerData.length > 0) {
    ...
    const { error: yieldError } = await supabase.from("yield_predictions").insert(yieldPredictions);
  ...
  const { error: cooperativeError } = await supabase.from("cooperatives").insert(cooperatives);
...
async function seedLogistics(supabase: SupabaseClient, userId: string) {
  // Vehicles
  const vehicles = [
    {
      plate: "LAG-241-KJA",
...
async function seedIntelligence(supabase: SupabaseClient, userId: string) {
  // Datasets
  const datasets = [
    {
...
async function seedInnovationLab(supabase: SupabaseClient, userId: string) {
  // Ideas
  const ideas = [
    {
  ...
  if (ideaData && ideaData.length > 0) {
    const prototypes = [
      ...
      },
      {
        ...
        status: "pilot",
        screenshots: JSON.stringify(["dashboard1.jpg", "analysis1.jpg"]),
      },
    ];

    const { error: prototypeError } = await supabase.from("prototypes").insert(prototypes);
  ...
  const { error: partnerError } = await supabase.from("partners").insert(partners);
...
Deno.serve(async (req) => {
  try {
    ...
    const { user_id, email, selected_divisions, primary_division, role_preference }: SignupPayload =
      await req.json();
    ...
    const { error: prefsError } = await supabaseClient.from("user_preferences").upsert({
      user_id,
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Workspaces created successfully" }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
      },
