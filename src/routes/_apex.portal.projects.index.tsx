...
import { listProjects, createProject } from "@/server/portal.functions";
    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-muted-foreground text-sm">Tech builds, properties and shipments — one model.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="bg-gold text-gold-foreground hover:bg-gold/90"><Plus className="h-4 w-4 mr-2" /> New project</Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <form onSubmit={onCreate} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="name" required maxLength={150} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              </div>
              <div className="space-y-2">
                <Label>Type</Label>
                <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v as Project["type"] })}>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="desc" maxLength={2000} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
              </div>
              <Button disabled={submitting} type="submit" className="w-full bg-gold text-gold-foreground hover:bg-gold/90">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((p) => (
            <Link key={p.id} to="/portal/projects/$id" params={{ id: p.id }} className="rounded-xl border border-border bg-surface p-5 hover:border-gold/40 transition">
              ...
              <div className="mt-4 text-xs text-muted-foreground capitalize">Status: {p.status}</div>
            </Link>
          ))}
        </div>
      )}
