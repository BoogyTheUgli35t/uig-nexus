...
for (const row of data) {
  if (/^https?:\/\//.test(row.storage_path)) urls.set(row.storage_path, `property_images:${row.id}`);
}
...
for (let i = 0; i < entries.length; i += 10) {
  await Promise.all(entries.slice(i, i + 10).map(async ([url, slot]) => {
    try {
      const res = await fetch(url, { method: "HEAD" });
      if (!res.ok) { failed++; console.error(`${res.status}  ${slot}  ${url}`); }
    } catch (e) { failed++; console.error(`ERR  ${slot}  ${url}  ${e.message}`); }
  }));
}
